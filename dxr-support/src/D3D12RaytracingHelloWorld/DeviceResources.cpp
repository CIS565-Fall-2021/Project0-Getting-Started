...
DeviceResources::DeviceResources(DXGI_FORMAT backBufferFormat,
                                 ...
                                 D3D_FEATURE_LEVEL minFeatureLevel, UINT flags,
...
void DeviceResources::CreateDeviceResources() {
  ...
  ThrowIfFailed(D3D12CreateDevice(m_adapter.Get(), m_d3dMinFeatureLevel,
  ...
  HRESULT hr = m_d3dDevice->CheckFeatureSupport(
  ...
  ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(
      &rtvDescriptorHeapDesc, IID_PPV_ARGS(&m_rtvDescriptorHeap)));
  ...
  if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN) {
    ...
    ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(
        &dsvDescriptorHeapDesc, IID_PPV_ARGS(&m_dsvDescriptorHeap)));
  }
...
void DeviceResources::CreateWindowSizeDependentResources() {
  if (!m_window) {
    ThrowIfFailed(E_HANDLE,
                  L"Call SetWindow with a valid Win32 window handle.\n");
  }
  ...
  if (m_swapChain) {
    ...
    HRESULT hr = m_swapChain->ResizeBuffers(
        m_backBufferCount, backBufferWidth, backBufferHeight, backBufferFormat,
    ...
    // DXGI does not allow creating a swapchain targeting a window which has
    // fullscreen styles(no border + topmost). Temporarily remove the topmost
    // property for creating the swapchain.
    ...
    ThrowIfFailed(m_dxgiFactory->CreateSwapChainForHwnd(
        m_commandQueue.Get(), m_window, &swapChainDesc, &fsSwapChainDesc,
        nullptr, &swapChain));
    ...
    // window message loop rather than let DXGI handle it by calling
    // SetFullscreenState.
  ...
  for (UINT n = 0; n < m_backBufferCount; n++) {
    ...
    CD3DX12_CPU_DESCRIPTOR_HANDLE rtvDescriptor(
        m_rtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), n,
        m_rtvDescriptorSize);
    m_d3dDevice->CreateRenderTargetView(m_renderTargets[n].Get(), &rtvDesc,
                                        rtvDescriptor);
  }
  ...
  if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN) {
    ...
    ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
        &depthHeapProperties, D3D12_HEAP_FLAG_NONE, &depthStencilDesc,
...
void DeviceResources::Prepare(D3D12_RESOURCE_STATES beforeState) {
  ...
  if (beforeState != D3D12_RESOURCE_STATE_RENDER_TARGET) {
    ...
    D3D12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(
        m_renderTargets[m_backBufferIndex].Get(), beforeState,
...
void DeviceResources::MoveToNextFrame() {
  ...
  if (m_fence->GetCompletedValue() < m_fenceValues[m_backBufferIndex]) {
    ThrowIfFailed(m_fence->SetEventOnCompletion(
        m_fenceValues[m_backBufferIndex], m_fenceEvent.Get()));
    WaitForSingleObjectEx(m_fenceEvent.Get(), INFINITE, FALSE);
  }
...
void DeviceResources::InitializeAdapter(IDXGIAdapter1** ppAdapter) {
  *ppAdapter = nullptr;
  ...
  for (UINT adapterID = 0; DXGI_ERROR_NOT_FOUND !=
                           m_dxgiFactory->EnumAdapters1(adapterID, &adapter);
       ++adapterID) {
    ...
    // actual device yet.
    if (SUCCEEDED(D3D12CreateDevice(adapter.Get(), m_d3dMinFeatureLevel,
