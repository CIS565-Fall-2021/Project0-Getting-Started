...
public:
  ShaderTable(ID3D12Device* device, UINT numShaderRecords,
              UINT shaderRecordSize, LPCWSTR resourceName = nullptr)
      : m_name(resourceName) {
    m_shaderRecordSize =
        Align(shaderRecordSize, D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
  ...
  void DebugPrint(std::unordered_map<void*, std::wstring> shaderIdToStringMap) {
    ...
    wstr << L"|----------------------------------------------------------------"
            L"----\n";
    wstr << L"|Shader table - " << m_name.c_str() << L": " << m_shaderRecordSize
         << L" | " << m_shaderRecords.size() * m_shaderRecordSize
         << L" bytes\n";

    for (UINT i = 0; i < m_shaderRecords.size(); i++) {
      wstr << L"| [" << i << L"]: ";
      wstr << shaderIdToStringMap[m_shaderRecords[i].shaderIdentifier.ptr]
           << L", ";
      wstr << m_shaderRecords[i].shaderIdentifier.size << L" + "
           << m_shaderRecords[i].localRootArguments.size << L" bytes \n";
    }
    wstr << L"|----------------------------------------------------------------"
            L"----\n";
    wstr << L"\n";
  ...
  for (UINT i = 0; i < desc->NumSubobjects; i++) {
    ...
    switch (desc->pSubobjects[i].Type) {
      case D3D12_STATE_SUBOBJECT_TYPE_GLOBAL_ROOT_SIGNATURE:
        wstr << L"Global Root Signature 0x" << desc->pSubobjects[i].pDesc
             << L"\n";
        break;
...
inline bool EnableComputeRaytracingFallback(IDXGIAdapter1* adapter) {
  ...
  return SUCCEEDED(D3D12EnableExperimentalFeatures(1, experimentalFeatures,
                                                   nullptr, nullptr)) &&
...
inline bool IsDirectXRaytracingSupported(IDXGIAdapter1* adapter) {
  ...
  return SUCCEEDED(D3D12CreateDevice(adapter, D3D_FEATURE_LEVEL_11_0,
         ...
         SUCCEEDED(testDevice->CheckFeatureSupport(
             D3D12_FEATURE_D3D12_OPTIONS5, &featureSupportData,
