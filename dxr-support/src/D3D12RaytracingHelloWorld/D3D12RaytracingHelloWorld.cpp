...
const wchar_t* D3D12RaytracingHelloWorld::c_raygenShaderName =
    L"MyRaygenShader";
const wchar_t* D3D12RaytracingHelloWorld::c_closestHitShaderName =
    L"MyClosestHitShader";
const wchar_t* D3D12RaytracingHelloWorld::c_missShaderName = L"MyMissShader";

D3D12RaytracingHelloWorld::D3D12RaytracingHelloWorld(UINT width, UINT height,
                                                     std::wstring name)
...
void D3D12RaytracingHelloWorld::EnableDirectXRaytracing(
    IDXGIAdapter1* adapter) {
  // Fallback Layer uses an experimental feature and needs to be enabled before
  // creating a D3D12 device.
  bool isFallbackSupported = EnableComputeRaytracingFallback(adapter);

  if (!isFallbackSupported) {
    OutputDebugString(
        L"Warning: Could not enable Compute Raytracing Fallback "
        L"(D3D12EnableExperimentalFeatures() failed).\n"
  ...
  if (!m_isDxrSupported) {
    OutputDebugString(
        L"Warning: DirectX Raytracing is not supported by your GPU and "
        L"driver.\n\n");

    ThrowIfFalse(isFallbackSupported,
                 L"Could not enable compute based fallback raytracing support "
                 L"(D3D12EnableExperimentalFeatures() failed).\n"
                 L"Possible reasons: your OS is not in developer mode.\n\n");
...
void D3D12RaytracingHelloWorld::OnInit() {
  m_deviceResources = std::make_unique<DeviceResources>(
      DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_FORMAT_UNKNOWN, FrameCount,
      D3D_FEATURE_LEVEL_11_0,
      // Sample shows handling of use cases with tearing support, which is OS
      // dependent and has been supported since TH2. Since the Fallback Layer
      // requires Fall Creator's update (RS3), we don't need to handle
      // non-tearing cases.
...
void D3D12RaytracingHelloWorld::SerializeAndCreateRaytracingRootSignature(
  ...
  if (m_raytracingAPI == RaytracingAPI::FallbackLayer) {
    ThrowIfFailed(
        m_fallbackDevice->D3D12SerializeRootSignature(
            &desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, &error),
        error ? static_cast<wchar_t*>(error->GetBufferPointer()) : nullptr);
    ThrowIfFailed(m_fallbackDevice->CreateRootSignature(
...
void D3D12RaytracingHelloWorld::CreateRootSignatures() {
  ...
  // invoked during a DispatchRays() call.
  {
    ...
    rootParameters[GlobalRootSignatureParams::OutputViewSlot]
        .InitAsDescriptorTable(1, &UAVDescriptor);
    rootParameters[GlobalRootSignatureParams::AccelerationStructureSlot]
...
void D3D12RaytracingHelloWorld::CreateRaytracingInterfaces() {
  ...
  if (m_raytracingAPI == RaytracingAPI::FallbackLayer) {
    ...
    ThrowIfFailed(D3D12CreateRaytracingFallbackDevice(
        device, createDeviceFlags, 0, IID_PPV_ARGS(&m_fallbackDevice)));
    m_fallbackDevice->QueryRaytracingCommandList(
        commandList, IID_PPV_ARGS(&m_fallbackCommandList));
  } else  // DirectX Raytracing
  {
    ThrowIfFailed(
        device->QueryInterface(IID_PPV_ARGS(&m_dxrDevice)),
        L"Couldn't get DirectX Raytracing interface for the device.\n");
    ThrowIfFailed(
        commandList->QueryInterface(IID_PPV_ARGS(&m_dxrCommandList)),
...
void D3D12RaytracingHelloWorld::CreateLocalRootSignatureSubobjects(
    CD3D12_STATE_OBJECT_DESC* raytracingPipeline) {
  // Hit group and miss shaders in this sample are not using a local root
  ...
  // Local root signature to be used in a ray gen shader.
  {
    auto localRootSignature =
        raytracingPipeline
            ->CreateSubobject<CD3D12_LOCAL_ROOT_SIGNATURE_SUBOBJECT>();
...
// with all configuration options resolved, such as local signatures and other
// state.
void D3D12RaytracingHelloWorld::CreateRaytracingPipelineStateObject() {
  // Create 7 subobjects that combine into a RTPSO:
  // Subobjects need to be associated with DXIL exports (i.e. shaders) either by
  // way of default or explicit associations. Default association applies to
  // every exported shader entrypoint that doesn't have any of the same type of
  // subobject associated with it. This simple sample utilizes default shader
  // association except for local root signature subobject which has an explicit
  // association specified purely for demonstration purposes. 1 - DXIL library
  ...
  auto lib =
      raytracingPipeline.CreateSubobject<CD3D12_DXIL_LIBRARY_SUBOBJECT>();
  D3D12_SHADER_BYTECODE libdxil =
      CD3DX12_SHADER_BYTECODE((void*)g_pRaytracing, ARRAYSIZE(g_pRaytracing));
  lib->SetDXILLibrary(&libdxil);
  // Define which shader exports to surface from the library.
  // If no shader exports are defined for a DXIL library subobject, all shaders
  // will be surfaced. In this sample, this could be omitted for convenience
  // since the sample uses all shaders in the library.
  {
  ...
  // Triangle hit group
  // A hit group specifies closest hit, any hit and intersection shaders to be
  // executed when a ray intersects the geometry's triangle/AABB. In this
  // sample, we only use triangle geometry with a closest hit shader, so others
  // are not set.
  auto hitGroup =
      raytracingPipeline.CreateSubobject<CD3D12_HIT_GROUP_SUBOBJECT>();
  ...
  auto shaderConfig =
      raytracingPipeline
          .CreateSubobject<CD3D12_RAYTRACING_SHADER_CONFIG_SUBOBJECT>();
  ...
  auto globalRootSignature =
      raytracingPipeline
          .CreateSubobject<CD3D12_GLOBAL_ROOT_SIGNATURE_SUBOBJECT>();
  ...
  auto pipelineConfig =
      raytracingPipeline
          .CreateSubobject<CD3D12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT>();
  ...
  if (m_raytracingAPI == RaytracingAPI::FallbackLayer) {
    ThrowIfFailed(m_fallbackDevice->CreateStateObject(
                      raytracingPipeline, IID_PPV_ARGS(&m_fallbackStateObject)),
                  L"Couldn't create DirectX Raytracing state object.\n");
  } else  // DirectX Raytracing
  {
    ThrowIfFailed(m_dxrDevice->CreateStateObject(
...
void D3D12RaytracingHelloWorld::CreateRaytracingOutputResource() {
  ...
  auto uavDesc = CD3DX12_RESOURCE_DESC::Tex2D(
      backbufferFormat, m_width, m_height, 1, 1, 1, 0,
...
void D3D12RaytracingHelloWorld::BuildGeometry() {
  ...
  Vertex vertices[] = {// The sample raytraces in screen space coordinates.
                       // Since DirectX screen space coordinates are right
                       // handed (i.e. Y axis points down). Define the vertices
                       // in counter clockwise order ~ clockwise in left handed.
                       {0, -offset, depthValue},
...
void D3D12RaytracingHelloWorld::BuildAccelerationStructures() {
  ...
  geometryDesc.Triangles.IndexCount =
      static_cast<UINT>(m_indexBuffer->GetDesc().Width) / sizeof(Index);
  geometryDesc.Triangles.IndexFormat  = DXGI_FORMAT_R16_UINT;
  geometryDesc.Triangles.Transform3x4 = 0;
  geometryDesc.Triangles.VertexFormat = DXGI_FORMAT_R32G32B32_FLOAT;
  geometryDesc.Triangles.VertexCount =
      static_cast<UINT>(m_vertexBuffer->GetDesc().Width) / sizeof(Vertex);
  geometryDesc.Triangles.VertexBuffer.StartAddress =
      m_vertexBuffer->GetGPUVirtualAddress();
  geometryDesc.Triangles.VertexBuffer.StrideInBytes = sizeof(Vertex);

  // Mark the geometry as opaque.
  // PERFORMANCE TIP: mark geometry as opaque whenever applicable as it can
  // enable important ray processing optimizations. Note: When rays encounter
  // opaque geometry an any hit shader will not be executed whether it is
  // present or not.
  ...
  // ALLOW_UNORDERED_ACCESS requirement simply acknowledges both:
  //  - the system will be doing this type of access in its implementation of
  //  acceleration structure builds behind the scenes.
  //  - from the app point of view, synchronization of writes/reads to
  //  acceleration structures is accomplished using UAV barriers.
  {
    ...
    AllocateUAVBuffer(device, bottomLevelPrebuildInfo.ResultDataMaxSizeInBytes,
                      &m_bottomLevelAccelerationStructure, initialResourceState,
                      L"BottomLevelAccelerationStructure");
    AllocateUAVBuffer(device, topLevelPrebuildInfo.ResultDataMaxSizeInBytes,
                      &m_topLevelAccelerationStructure, initialResourceState,
                      L"TopLevelAccelerationStructure");
  }

  // Note on Emulated GPU pointers (AKA Wrapped pointers) requirement in
  // Fallback Layer: The primary point of divergence between the DXR API and the
  // compute-based Fallback layer is the handling of GPU pointers. DXR
  // fundamentally requires that GPUs be able to dynamically read from arbitrary
  // addresses in GPU memory. The existing Direct Compute API today is more
  // rigid than DXR and requires apps to explicitly inform the GPU what blocks
  // of memory it will access with SRVs/UAVs. In order to handle the
  // requirements of DXR, the Fallback Layer uses the concept of Emulated GPU
  // pointers, which requires apps to create views around all memory they will
  // access for raytracing, but retains the DXR-like flexibility of only needing
  // to bind the top level acceleration structure at DispatchRays.
  //
  // The Fallback Layer interface uses WRAPPED_GPU_POINTER to encapsulate the
  // underlying pointer which will either be an emulated GPU pointer for the
  // compute - based path or a GPU_VIRTUAL_ADDRESS for the DXR path.
  ...
  if (m_raytracingAPI == RaytracingAPI::FallbackLayer) {
    ...
    UINT numBufferElements =
        static_cast<UINT>(bottomLevelPrebuildInfo.ResultDataMaxSizeInBytes) /
        sizeof(UINT32);
    instanceDesc.AccelerationStructure = CreateFallbackWrappedPointer(
        m_bottomLevelAccelerationStructure.Get(), numBufferElements);
  ...
  if (m_raytracingAPI == RaytracingAPI::FallbackLayer) {
    UINT numBufferElements =
        static_cast<UINT>(topLevelPrebuildInfo.ResultDataMaxSizeInBytes) /
        sizeof(UINT32);
    m_fallbackTopLevelAccelerationStructurePointer =
        CreateFallbackWrappedPointer(m_topLevelAccelerationStructure.Get(),
                                     numBufferElements);
  }
  ...
  if (m_raytracingAPI == RaytracingAPI::FallbackLayer) {
    ...
    m_fallbackCommandList->SetDescriptorHeaps(ARRAYSIZE(pDescriptorHeaps),
                                              pDescriptorHeaps);
  ...
  // Wait for GPU to finish as the locally created temporary GPU resources will
  // get released once we go out of scope.
...
void D3D12RaytracingHelloWorld::BuildShaderTables() {
  ...
  auto GetShaderIdentifiers = [&](auto* stateObjectProperties) {
    rayGenShaderIdentifier =
        stateObjectProperties->GetShaderIdentifier(c_raygenShaderName);
    missShaderIdentifier =
        stateObjectProperties->GetShaderIdentifier(c_missShaderName);
    hitGroupShaderIdentifier =
        stateObjectProperties->GetShaderIdentifier(c_hitGroupName);
  };
  ...
  // Ray gen shader table
  {
    ...
    rayGenShaderTable.push_back(
        ShaderRecord(rayGenShaderIdentifier, shaderIdentifierSize,
                     &rootArguments, sizeof(rootArguments)));
  ...
  // Miss shader table
  {
    ...
    missShaderTable.push_back(
        ShaderRecord(missShaderIdentifier, shaderIdentifierSize));
  ...
  // Hit group shader table
  {
    ...
    hitGroupShaderTable.push_back(
        ShaderRecord(hitGroupShaderIdentifier, shaderIdentifierSize));
...
void D3D12RaytracingHelloWorld::DoRaytracing() {
  ...
  auto DispatchRays = [&](auto* commandList, auto* stateObject,
                          auto* dispatchDesc) {
    // Since each shader table has only one shader record, the stride is same as
    // the size.
  ...
  if (m_raytracingAPI == RaytracingAPI::FallbackLayer) {
    m_fallbackCommandList->SetDescriptorHeaps(1,
                                              m_descriptorHeap.GetAddressOf());
    commandList->SetComputeRootDescriptorTable(
        GlobalRootSignatureParams::OutputViewSlot,
        m_raytracingOutputResourceUAVGpuDescriptor);
    m_fallbackCommandList->SetTopLevelAccelerationStructure(
        GlobalRootSignatureParams::AccelerationStructureSlot,
        m_fallbackTopLevelAccelerationStructurePointer);
    DispatchRays(m_fallbackCommandList.Get(), m_fallbackStateObject.Get(),
                 &dispatchDesc);
  } else  // DirectX Raytracing
  {
    D3D12_DISPATCH_RAYS_DESC dispatchDesc = {};
...
void D3D12RaytracingHelloWorld::UpdateForSizeChange(UINT width, UINT height) {
  ...
  float border = 0.1f;
  if (m_width <= m_height) {
    m_rayGenCB.stencil = {-1 + border, -1 + border * m_aspectRatio,
                          1.0f - border, 1 - border * m_aspectRatio};
  } else {
    m_rayGenCB.stencil = {-1 + border / m_aspectRatio, -1 + border,
...
void D3D12RaytracingHelloWorld::CopyRaytracingOutputToBackbuffer() {
  ...
  preCopyBarriers[1] = CD3DX12_RESOURCE_BARRIER::Transition(
      m_raytracingOutput.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
  ...
  postCopyBarriers[1] = CD3DX12_RESOURCE_BARRIER::Transition(
      m_raytracingOutput.Get(), D3D12_RESOURCE_STATE_COPY_SOURCE,
...
void D3D12RaytracingHelloWorld::CalculateFrameStats() {
  ...
  if ((totalTime - elapsedTime) >= 1.0f) {
    ...
    float fps =
        static_cast<float>(frameCnt) / diff;  // Normalize to an exact second.
    ...
    windowText << setprecision(2) << fixed << L"    fps: " << fps
               ...
               << L"    GPU[" << m_deviceResources->GetAdapterID() << L"]: "
...
void D3D12RaytracingHelloWorld::OnSizeChanged(UINT width, UINT height,
                                              bool minimized) {
...
WRAPPED_GPU_POINTER D3D12RaytracingHelloWorld::CreateFallbackWrappedPointer(
  ...
  // Only compute fallback requires a valid descriptor index when creating a
  // wrapped pointer.
  UINT descriptorHeapIndex = 0;
  if (!m_fallbackDevice->UsingRaytracingDriver()) {
    descriptorHeapIndex = AllocateDescriptor(&bottomLevelDescriptor);
    device->CreateUnorderedAccessView(resource, nullptr, &rawBufferUavDesc,
                                      bottomLevelDescriptor);
  }
  return m_fallbackDevice->GetWrappedPointerSimple(
      descriptorHeapIndex, resource->GetGPUVirtualAddress());
}

// Allocate a descriptor and return its index.
// If the passed descriptorIndexToUse is valid, it will be used instead of
// allocating a new one.
UINT D3D12RaytracingHelloWorld::AllocateDescriptor(
