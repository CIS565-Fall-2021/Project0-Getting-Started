...
inline void ThrowIfFalse(bool value) { ThrowIfFailed(value ? S_OK : E_FAIL); }
...
protected:
  ...
  void Allocate(ID3D12Device* device, UINT bufferSize,
  ...
  uint8_t* MapCpuWriteOnly() {
    uint8_t* mappedData;
    // We don't unmap this until the app closes. Keeping buffer mapped for the
...
// Helper class to create and update a constant buffer with proper constant
// buffer alignments. Usage: ToDo
//    ConstantBuffer<...> cb;
//    cb.Create(...);
...
public:
  ...
  void Create(ID3D12Device* device, UINT numInstances = 1,
              LPCWSTR resourceName = nullptr) {
  ...
  void CopyStagingToGpu(UINT instanceIndex = 0) {
    memcpy(m_mappedConstantData + instanceIndex * m_alignedInstanceSize,
           &staging, sizeof(T));
  }
  ...
  D3D12_GPU_VIRTUAL_ADDRESS GpuVirtualAddress(UINT instanceIndex = 0) {
    return m_resource->GetGPUVirtualAddress() +
           instanceIndex * m_alignedInstanceSize;
  }
};
...
public:
  ...
  static_assert(
      sizeof(T) % 16 == 0,
      L"Align structure buffers on 16 byte boundary for performance reasons.");

  StructuredBuffer() : m_mappedBuffers(nullptr), m_numInstances(0) {}

  void Create(ID3D12Device* device, UINT numElements, UINT numInstances = 1,
              LPCWSTR resourceName = nullptr) {
